...
def submit_custom_result(payload):
    ...
    # Keep this same Streamlit run alive so the Local Storage writer can
    # render before the player leaves the result page.
    save_progress()
...
<div class="game-shell">
    ...
    <div class="sequence-title">STORY SEQUENCE</div><div class="sequence-help">Click a picture in the Story Tray, then click a Scenario box to place it.</div><div class="tray-title">STORY TRAY</div>
    ...
    node.addEventListener("click", event => {{
        ...
        if (selectedCardId === card.id) {{
            ...
            document.querySelectorAll(".slot")
                .forEach(element => element.classList.remove("click-ready"));
            message.textContent = "";
            return;
        }}
        ...
        document.querySelectorAll(".slot")
            .forEach(element => element.classList.add("click-ready"));

        message.textContent =
            "Picture selected. Click the correct Scenario box.";
    }});
...
function placeCardInTarget(cardId, body, isTray) {{
    if (!cardId) return;
...
function createSlot(index) {{
    ...
    slot.addEventListener("click", () => {{
        if (selectedCardId) {{
            placeCardInTarget(selectedCardId, body, false);
        }}
    }});
...
function makeDropTarget(target, body, isTray) {{
    ...
    target.addEventListener("drop", event => {{
        ...
        placeCardInTarget(cardId, body, isTray);
    }});
}}
...
tray.addEventListener("click", event => {{
    if (selectedCardId && event.target === tray) {{
        placeCardInTarget(selectedCardId, tray, true);
    }}
}});
...
if custom_submit_value:
    try:
        ...
        custom_result_processed = True

        # Write the completed progress immediately in this same run.
        flush_pending_browser_action()
...
if custom_result_processed:
    st.session_state.screen = "result"
elif query_screen in VALID_SCREENS:
...
if screen == "home":
    ...
    st.markdown(
        (
            ...
            f'</div>'
            '<p>'
            'Click and drop one picture into each scenario, '
            'then answer every scenario question.'
            '</p>'
            '</div>'
        ),
        unsafe_allow_html=True,
    ...
    st.markdown(
        (
            ...
            f'<h2>{level["title"]}</h2>'
            '<p>'
            'Click a picture in the Story Tray, then click the correct '
            'Scenario to place it. You can also drag and drop. '
            'Scenario questions appear when selected pictures are placed.'
            '</p>'
            '</div>'
        ),
        unsafe_allow_html=True,
